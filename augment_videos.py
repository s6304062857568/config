...
def augmentVideo(annotation_dict, labels_dict, data_dir='/content/dataset_videos/examples/', output_dir='/content/dataset_videos/augmented-examples/'):
    """
    Takes in annotation dictionary, labels dictionary, path to the directory of data, and the output directory
    :param annotation_dict: Dictionary of human action annotations
    :param labels_dict: Dictionary matching the encoding to a Human Action
    :param data_dir: Path to Human Action Video Data
    ...
    with open(annotation_dict) as f:
        ...
        annotation_dict = {str(k): int(annotation_dict[k]) for k in annotation_dict}
        video_list = list(annotation_dict.items())

    with open(labels_dict) as f:
        labels_dict = json.load(f)
        labels_dict = {int(k): labels_dict[k] for k in labels_dict}
    ...
    with open('/content/dataset_videos/augmented_annotation_dict.json', 'w') as fp:
...
if __name__ == "__main__":
    annotation_dict = "/content/dataset_videos/annotation_dict.json"
    labels_dict = "/content/dataset_videos/labels_dict.json"
